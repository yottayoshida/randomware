...
Radio Browser is the one explicit variable-host exception. The broker resolves the station stream from a Radio Browser response, rejects credentials, IP literals, loopback/local hostnames, and non-audio schemes, stores the exact resolved URL, and returns a short-lived signed `/media` URL. The media route permits one concurrent stream, five minutes, and 8 MiB per page load. Generated code cannot provide or alter the upstream URL. Community-edited stream destinations remain the accepted PRD residual risk.
