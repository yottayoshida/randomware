...
| `wikimedia-commons-audio` | [Wikimedia Commons audio](https://www.mediawiki.org/wiki/API:Imageinfo) | owner probe HTTP 200 in 0.85 s; implementation fixture HTTP 200 | Fixed four-result query; only the 2,186,836-byte recording was at or below 8 MiB (the other captured candidates were 10,471,718, 33,264,080, and 585,836,712 bytes) | Promote as the selectable second audio source. Query uses `field recording filetype:audio filesize:>100`, preserves bounded `LicenseShortName`, probes before signing, and permits media only on exact `upload.wikimedia.org`. Owner separately measured a 940,416-byte `audio/mpeg` file at 1.5 s. |
| `librivox` | [LibriVox](https://librivox.org/api/info) | disabled by owner | RSS resolution measured 12.6 s; media TTFB 13–18 s; book `id=47` exposes a roughly 20 MB enclosure that produced `media_bytes_cap` | Permanently remove from the selection pool. Keep the registry entry and adapter only for frozen-creation runtime compatibility. The two-stage `librivox.org` → `archive.org` path adds an avoidable failure surface and structurally conflicts with the 8 MiB page-load media cap. |
...
The resulting registry has 21 compatibility entries and exactly 20 selectable launch entries: GBIF failed its explicit bounded probe, LibriVox remains preserved but selection-disabled, and Wikimedia Commons Audio takes its roster slot. NASA, LOC, and Commons have raw fixtures, production-adapted goldens, model-visible response examples, exact asset/media policies, policy metadata, symbols, selector/health integration, and fixture parity coverage.
