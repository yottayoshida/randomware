...
Complete PLAN Milestone 4: implementation-time terms/attribution/health check, 20-entry selectable launch set plus retained disabled compatibility entries, fixed typed operations/adapters, fixtures, health tooling, weighting/distribution tests, and labeled replay fallback. Record every enabled/disabled/promoted API reason. Continue with 12–20 healthy selectable entries; stop below 10. Keep ISS excluded and iTunes spike-only.
