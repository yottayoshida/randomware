...
test('MCP surface exposes eight annotated tools', async (t) => {
  ...
  assert.ok(result.body.result.tools.every((tool) => tool.description.startsWith('TOP PRIORITY — PLAYABILITY')));
  assert.ok(result.body.result.tools.every((tool) => tool.description.includes('BEST WITH GPT-5.6 SOL (HIGH REASONING)')));
