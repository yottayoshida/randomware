...
test('generated runtime is contained and unknown calls are denied', async (t) => {
  ...
  const runtimeHtml = await runtime.text();
  assert.match(runtimeHtml, /window\.randomware/);
  const tokenLiteral = runtimeHtml.match(/capability:("(?:\\.|[^"])*")/);
  assert.ok(tokenLiteral, 'trusted harness must bind a capability');
  const capability = JSON.parse(tokenLiteral[1]);
