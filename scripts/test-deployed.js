...
(async () => {
  ...
  const resourceRead = await mcp({ jsonrpc: '2.0', id: 4, method: 'resources/read', params: { uri: 'ui://widget/randomware.html' } }); const resourceReadBody = await resourceRead.json(); const content = resourceReadBody.result?.contents?.[0]; if (content?.mimeType !== 'text/html;profile=mcp-app' || !content?._meta?.ui?.csp?.frameDomains?.includes(base) || !content.text.includes('id="creation-frame"') || !content.text.includes('/c/') || !content.text.includes('openExternal({href') || !content.text.includes('structuredContent') || !content.text.includes('openai:set_globals') || !content.text.includes('The build needs one bounded repair') || !content.text.includes('sendFollowUpMessage') || !content.text.includes('id="build-prompt"') || !content.text.includes('id="resume-timer"') || !content.text.includes('Follow-up unavailable') || !content.text.includes('statusUrl') || !content.text.includes('setInterval')) throw new Error('mcp_resource_read_failed');
