...
(async () => {
  ...
  const mediaSpun = await call(18, 'spin_apis', { seed: 'media-1', requestId: `${testTag}-media-spin` }); const mediaRun = mediaSpun.structuredContent; const audioApis = mediaRun.selectedApis.filter((api) => ['radio-browser', 'librivox'].includes(api.id)); if (audioApis.length !== 2) throw new Error(`audio_selection_failed:${mediaRun.selectedApis.map((api) => api.id).join(',')}`);
  await call(19, 'submit_concept', conceptFor(mediaRun, 'audio', `${testTag}-audio-concept`)); const mediaArtifact = await call(20, 'submit_artifact', { runId: mediaRun.runId, requestId: `${testTag}-audio-artifact`, html: artifactFor(mediaRun, 'audio') }); if (mediaArtifact.structuredContent?.phase !== 'completed') throw new Error('audio_artifact_failed');
