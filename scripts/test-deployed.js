...
(async () => {
  ...
  const creation = await fetch(`${base}/c/${creationId}`); const creationHtml = await creation.text(); if (!creation.ok || !creationHtml.includes('sandbox="allow-scripts"') || !creationHtml.includes('min-height:390px')) throw new Error('creation_surface_failed');
  const creationCsp = creation.headers.get('content-security-policy') || ''; if (!creationCsp.includes('frame-ancestors https://chatgpt.com https://chat.openai.com')) throw new Error('creation_frame_ancestors_failed');
