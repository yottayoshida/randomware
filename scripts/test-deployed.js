...
(async () => {
  ...
  if (!mediaRun) throw new Error('audio_selection_failed');
  const audioApis = mediaRun.selectedApis.filter((api) => api.id === 'radio-browser');
  ...
  for (const audioApi of audioApis) {
    const operation = audioApi.operations[0]; let audioCall; let audioFailure = ''; for (let attempt = 0; attempt < 2; attempt += 1) { audioCall = await fetch(`${base}/api/runtime/call`, { method: 'POST', headers: { Origin: 'null', 'Content-Type': 'application/json' }, body: JSON.stringify({ creationId: mediaCreationId, revision: 1, apiId: audioApi.id, operationId: operation.id, params: {}, capability: mediaCapability }) }); if (audioCall.ok) break; audioFailure = await audioCall.text(); } if (!audioCall?.ok) throw new Error(`audio_broker_status:${audioApi.id}:${audioCall?.status}:${audioFailure}`); const audioBody = await audioCall.json(); const signedMedia = audioBody.data?.mediaUrl; if (!signedMedia || new URL(signedMedia).origin !== base || !new URL(signedMedia).pathname.startsWith('/media/')) throw new Error(`audio_signed_url_failed:${audioApi.id}`);
    const mediaOrigin = 'https://web-sandbox.oaiusercontent.com'; const mediaPreflight = await fetch(signedMedia, { method: 'OPTIONS', headers: { Origin: mediaOrigin, 'Access-Control-Request-Method': 'GET', 'Access-Control-Request-Headers': 'range' } }); if (mediaPreflight.status !== 204 || mediaPreflight.headers.get('access-control-allow-origin') !== '*' || !(mediaPreflight.headers.get('access-control-allow-methods') || '').includes('GET') || !(mediaPreflight.headers.get('access-control-allow-headers') || '').toLowerCase().includes('range')) throw new Error(`audio_cors_preflight_failed:${audioApi.id}:${mediaPreflight.status}`); const mediaResponse = await fetch(signedMedia, { headers: { Origin: mediaOrigin, Range: 'bytes=0-4095' } }); const mediaType = mediaResponse.headers.get('content-type') || ''; if (![200, 206].includes(mediaResponse.status) || mediaResponse.headers.get('access-control-allow-origin') !== '*' || !(mediaType.startsWith('audio/') || mediaType === 'application/ogg')) throw new Error(`audio_stream_failed:${audioApi.id}:${mediaResponse.status}:${mediaType}`); const reader = mediaResponse.body?.getReader(); if (!reader) throw new Error(`audio_stream_body_missing:${audioApi.id}`); const firstChunk = await reader.read(); if (firstChunk.done || !firstChunk.value?.byteLength) throw new Error(`audio_stream_empty:${audioApi.id}`); await reader.cancel(); mediaRequests.push({ apiId: audioApi.id, status: mediaResponse.status, contentType: mediaType, cors: mediaResponse.headers.get('access-control-allow-origin'), preflight: mediaPreflight.status, bytes: firstChunk.value.byteLength });
  }
  const imagePriority = ['rickandmorty', 'randomuser', 'deck-of-cards', 'open-food-facts', 'themealdb'];
