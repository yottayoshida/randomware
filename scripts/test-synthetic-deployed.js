...
function artifactFromVisibleContract(run, manifest) {
  ...
  const script = `const output=document.querySelector('#output');const audio=document.querySelector('#audio');const audioStatus=document.querySelector('#audio-status');const contracts=${JSON.stringify(operationContracts)};const read=(value,path)=>path.replace(/\\[(\\d+)\\]/g,'.$1').split('.').filter(Boolean).reduce((current,key)=>current==null?undefined:current[key],value);document.querySelector('#go').addEventListener('click',async()=>{output.textContent='Loading bounded signals…';audioStatus.textContent='TUNING THE CARRIER…';audio.hidden=true;audio.removeAttribute('src');const settled=await Promise.allSettled([${calls.join(',')}]);const rendered=[];const results=[];settled.forEach((item,index)=>{if(item.status==='rejected'){rendered.push(contracts[index].apiId+': Source unavailable: '+String(item.reason?.message||'broker_failure'));return}const result=item.value;if(!result||result.ok!==true||!Object.prototype.hasOwnProperty.call(result,'data')){rendered.push(contracts[index].apiId+': Source unavailable: broker_envelope_invalid');return}const value=read(result.data,contracts[index].path);if(value===undefined||value===null||value===''||Number.isNaN(value)){rendered.push(contracts[index].apiId+': Source unavailable: semantic_value_missing');return}results.push(result);rendered.push(contracts[index].apiId+': '+(typeof value==='object'?JSON.stringify(value):String(value)))});output.textContent=rendered.join('\\n');output.dataset.semantic=results.length?'complete':'degraded';const media=results.map((result)=>result.data?.mediaUrl).find(Boolean);if(media){audio.src=media;audio.hidden=false;audioStatus.textContent='Carrier ready. Press play.'}else{audioStatus.textContent='Source unavailable: signed mediaUrl missing.'}});${contract.ready};`;
  let html = `<!doctype html><html><head><meta charset="utf-8">${contract.viewport} content="width=device-width,initial-scale=1"><title>Signal Opera</title><style>:root{color-scheme:dark}*{box-sizing:border-box}body{margin:0;min-height:100vh;padding:24px;background:#100c24;color:#fff0c7;font:16px Georgia,serif}main{max-width:780px;margin:auto;border:3px solid #fff0c7;padding:28px;box-shadow:12px 12px 0 #e8614f}h1{font-size:clamp(3rem,12vw,7rem);line-height:.8}button{padding:14px 20px;background:#55e6c1;border:0;font-weight:800}pre{white-space:pre-wrap;border-left:4px solid #e8614f;padding:16px}audio{width:100%;margin-top:18px}</style></head><body><main><section ${markers.loading}>Signals are waiting.</section><h1>Signal Opera</h1><button id="go" type="button">Conduct the collision</button><section ${markers.interactive}><pre id="output" aria-live="polite">The stage is ready.</pre><p id="audio-status" aria-live="polite">Carrier idle. Conduct the collision to resolve audio.</p><audio id="audio" controls preload="none" hidden></audio></section><section ${markers.error}>A bounded failure will be shown in the output.</section><footer ${markers.attribution}>Randomware mediated API specimen.</footer></main><script>${script}</script></body></html>`;
