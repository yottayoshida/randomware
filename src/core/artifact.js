...
function createArtifact({ appName, selected }) {
  ...
  const script = `const out=document.querySelector('#result');const sources=${JSON.stringify(selected.map(({ apiId }) => apiId))};document.querySelector('#go').addEventListener('click',async()=>{out.dataset.state='loading';const settled=await Promise.allSettled([${calls}]);out.textContent=settled.map((item,index)=>item.status==='fulfilled'?sources[index]+': '+JSON.stringify(item.value.data):sources[index]+': Source unavailable: '+String(item.reason?.message||'broker_failure')).join('\\n');out.dataset.state='interactive'});${ARTIFACT_CONTRACT.ready};`;
