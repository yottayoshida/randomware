...
function showcasePage(runs) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Randomware — Real APIs go in</title><link rel="stylesheet" href="/styles.css"></head><body><header class="judge-strip" id="for-judges"><strong>For judges</strong><nav aria-label="Judge links"><a href="https://youtu.be/6aPYDkCDbuQ" target="_blank" rel="noopener noreferrer">Demo video</a><a href="${CONNECT_URL}">Connect in ChatGPT</a><a href="${REPOSITORY_URL}">GitHub repository</a></nav></header><main class="shell"><header class="masthead"><p class="eyebrow">A slot machine for software</p><h1>Randomware</h1><p class="tagline">Real APIs go in. Random apps come out.</p><p class="lede">Randomware collides bounded public APIs, asks a model for one causal absurdity, then publishes the sandboxed specimen with its receipts and failures intact.</p><a class="primary" href="${CONNECT_URL}">Spin it yourself →</a></header>${heroHtml}<section aria-labelledby="specimens-title"><p class="eyebrow">Curated showcase</p><h2 id="specimens-title">Specimens that earned shelf space</h2><div class="specimen-grid">${cards}</div></section><footer class="site-footer"><p>${registry.filter((entry) => entry.selectionEnabled !== false).length} selectable public sources · ${registry.filter((entry) => entry.selectionEnabled === false).length} preserved compatibility source. No machine-check specimens in the cabinet.</p><a href="/api/registry">Inspect registry metadata</a></footer></main></body></html>`;
}
