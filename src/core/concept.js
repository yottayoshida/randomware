const { BANNED_SHAPE_PHRASES, TOOL_SCHEMAS } = require('./artifact-contract');

const CONCEPT_SCHEMA = TOOL_SCHEMAS.submit_concept.properties;
const BANNED_SHAPES = BANNED_SHAPE_PHRASES.map((phrase) => new RegExp(phrase.replace(/\s+/g, '\\s+'), 'i'));

function fail(code, diagnostics = [code]) { return { ok: false, code, diagnostics }; }

function text(value, schema, label) {
  if (value === undefined || value === null) return `${label}_missing`;
  if (typeof value !== 'string') return `${label}_type`;
  if ((schema.minLength !== undefined && value.trim().length < schema.minLength) || (schema.maxLength !== undefined && value.trim().length > schema.maxLength)) return `${label}_length`;
  return null;
}
...
function validateConcept(concept, { selectedApis = [], prior = [], styleId = null } = {}) {
  try {
    ...
    roles.forEach((role, index) => {
      ...
      const roleError = text(role.essentialRole, CONCEPT_SCHEMA.apiRoles.items.properties.essentialRole, `api_role_essential_role:${roleId}`); if (roleError) errors.push(roleError);
    ...
    concept.causalChain.forEach((item, index) => {
      ...
      const actionError = text(item.action, CONCEPT_SCHEMA.causalChain.items.properties.action, `causal_chain_action:${index}`); if (actionError) errors.push(actionError);
    });
    ...
    if (!dependency || typeof dependency !== 'object' || Array.isArray(dependency)) errors.push('dependency_type');
      ...
      if (!CONCEPT_SCHEMA.dependency.properties.to.enum.includes(dependency.to)) errors.push('dependency_to_invalid');
      if (dependency.toApiId !== undefined && typeof dependency.toApiId !== 'string') errors.push('dependency_to_api_id_type');
      const dependencyExplanation = text(dependency.explanation, CONCEPT_SCHEMA.dependency.properties.explanation, 'dependency_explanation'); if (dependencyExplanation) errors.push(dependencyExplanation);
    ...
    if (!interaction || typeof interaction !== 'object' || Array.isArray(interaction)) errors.push('interaction_type');
      ...
      if (!Array.isArray(interaction.controls)) errors.push('interaction_controls_missing');
        ...
        const controlsSchema = CONCEPT_SCHEMA.interaction.properties.controls;
        if (interaction.controls.length < controlsSchema.minItems || interaction.controls.length > controlsSchema.maxItems) errors.push('interaction_controls_length');
        interaction.controls.forEach((control, index) => { if (typeof control !== 'string' || !control) errors.push(`interaction_control_type:${index}`); });
      }
      const outcomeError = text(interaction.outcome, CONCEPT_SCHEMA.interaction.properties.outcome, 'interaction_outcome'); if (outcomeError) errors.push(outcomeError);
    }
    ...
    else for (const key of CONCEPT_SCHEMA.visualDirection.required) { const visualError = text(visual[key], CONCEPT_SCHEMA.visualDirection.properties[key], `visual_${key}`); if (visualError) errors.push(visualError); }
    ...
    if (!assessment || typeof assessment !== 'object' || Array.isArray(assessment)) errors.push('banned_shape_assessment_type');
      ...
      for (const key of CONCEPT_SCHEMA.bannedShapeAssessment.required.filter((key) => key !== 'explanation')) if (assessment[key] !== CONCEPT_SCHEMA.bannedShapeAssessment.properties[key].const) errors.push(`banned_shape_${key}_must_be_false`);
      const assessmentError = text(assessment.explanation, CONCEPT_SCHEMA.bannedShapeAssessment.properties.explanation, 'banned_shape_explanation'); if (assessmentError) errors.push(assessmentError);
    }
