...
function scheduleTimer(){if(!timerState||timerState.paused)return;const phase=timerState.phase;const target=timerState.reSteered?timerState.finalAt:timerState.firstAt;timer=setTimeout(()=>{timer=null;if(!timerState||timerState.paused)return;if(!timerState.reSteered){timerState.reSteered=true;status.textContent='Still waiting for the '+phase+'…';save();void sendFollowUp(phase).catch((error)=>showFollowUpFallback(phase,error)).finally(()=>scheduleTimer())}else void recordTimeout()},Math.max(0,target-Date.now()))}
...
function startTimer(phase){const limits=${JSON.stringify(CHOREOGRAPHY_DEADLINES)}[phase];if(!limits||!run?.runId)return;clearTimer();const now=Date.now();timerState={phase,firstAt:now+limits.firstMs,finalAt:now+limits.finalMs,reSteered:false,paused:false};scheduleTimer();save()}
