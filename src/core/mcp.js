...
function showRun(value){run=value?.structuredContent||value||null;list.replaceChildren();build.hidden=true;updateTimerForRun();if(!run?.selectedApis?.length){status.textContent='The slot is ready.';return}status.textContent='The reels are revealing…';run.selectedApis.forEach((api,index)=>setTimeout(()=>{const chip=document.createElement('li');chip.textContent=text(api.name||api.id);list.append(chip);if(index===run.selectedApis.length-1){status.textContent='Collision selected. The model can now invent the causal chain.';build.hidden=false;save()}},index*420))}
