...
function progressIndex(value){if(value?.phase==='completed')return 4;if(value?.phase==='failed')return value?.revisions?.length?2:1;if(value?.phase==='concept_accepted'||value?.phase==='building'||value?.phase==='repair_requested')return 2;if(value?.phase==='spinned')return 1;return 0}
function renderProgress(){const index=progressIndex(run);steps.querySelectorAll('li').forEach((item,position)=>item.dataset.state=position<index?'done':position===index&&index<4?(run?.phase==='failed'?'failed':'current'):'pending');const active=(['concept_accepted','building','repair_requested'].includes(run?.phase)||(run?.phase==='spinned'&&buildEngaged))&&Boolean(activeWidgetRunId);composing.hidden=!active;const start=Number(run?.createdAt||runStartedAt);elapsed.textContent='elapsed '+(start?age(Date.now()-start):'0s');const activity=Number(run?.choreography?.lastActivityAt);heartbeat.textContent='last activity '+(activity?age(Date.now()-activity)+' ago':'—')}
