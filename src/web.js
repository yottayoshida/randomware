    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/mcp') {
        ...
        if (input.method === 'tools/call') {
          ...
          if (name === 'submit_concept') { const run = await callStore('getRun', args.runId); const concept = { ...args, apiIds: args.apiIds || run.selectedApis.map((entry) => entry.apiId) }; const check = validateConcept(concept, { selectedApis: run.selectedApis, prior: run.history || [] }); if (!check.ok) return response({ jsonrpc: '2.0', id: input.id, result: { isError: true, structuredContent: check } }); return response({ jsonrpc: '2.0', id: input.id, result: { structuredContent: summary(await callStore('acceptConcept', args.runId, concept)) } }); }
          if (name === 'submit_artifact' || name === 'submit_repair') { const run = await callStore('getRun', args.runId); const check = validateArtifact(args.html, { selectedApis: run.selectedApis }); if (!check.ok) { if (name === 'submit_repair') await callStore('recordRepairFailure', args.runId, { requestId: args.requestId || seed(), code: check.code, html: args.html }); else await callStore('recordArtifactFailure', args.runId, { requestId: args.requestId || seed(), code: check.code, html: args.html }); return response({ jsonrpc: '2.0', id: input.id, result: { isError: true, structuredContent: { ...check, nextTool: name === 'submit_repair' ? 'none' : 'submit_repair' } } }); } const accepted = name === 'submit_repair' ? await callStore('acceptRepair', args.runId, { requestId: args.requestId || seed(), html: args.html, sha256: check.sha256, bytes: check.bytes }) : await callStore('acceptArtifact', args.runId, { requestId: args.requestId || seed(), html: args.html, sha256: check.sha256, bytes: check.bytes }); return response({ jsonrpc: '2.0', id: input.id, result: { structuredContent: summary(accepted) } }); }
          if (name === 'mutate_creation') { const run = await callStore('findByCreation', args.creationId); return response({ jsonrpc: '2.0', id: input.id, result: { structuredContent: { ok: true, creationId: run.creationId, apiIds: run.selectedApis.map((entry) => entry.apiId), premise: args.premise, note: 'mutation preserves the immutable selected API set' } } }); }
          if (name === 'record_choreography_failure') { const failed = await callStore('fail', args.runId, args.code || 'choreography_timeout', args.phase); return response({ jsonrpc: '2.0', id: input.id, result: { structuredContent: summary(failed) } }); }
        }
      ...
      if (runMatch) {
        ...
        if (request.method === 'POST' && (action === 'artifact' || action === 'repair')) { const input = await readJson(request); const run = await callStore('getRun', runId); const check = validateArtifact(input.html, { selectedApis: run.selectedApis }); if (!check.ok) { if (action === 'repair') await callStore('recordRepairFailure', runId, { requestId: input.requestId || seed(), code: check.code, html: input.html }); else await callStore('recordArtifactFailure', runId, { requestId: input.requestId || seed(), code: check.code, html: input.html }); return response({ ...check, nextTool: action === 'repair' ? 'none' : 'submit_repair' }, 422); } const accepted = action === 'repair' ? await callStore('acceptRepair', runId, { requestId: input.requestId || seed(), html: input.html, sha256: check.sha256, bytes: check.bytes }) : await callStore('acceptArtifact', runId, { requestId: input.requestId || seed(), html: input.html, sha256: check.sha256, bytes: check.bytes }); return response({ ok: true, creationId: accepted.creationId, ...summary(accepted) }); }
      }
      const creation = url.pathname.match(/^\/(c|run)\/([^/]+)$/); if (creation && request.method === 'GET') { const run = await callStore('findByCreation', creation[2]); if (run.unpublished) return response('<h1>Creation removed</h1>', 200, 'text/html; charset=utf-8'); const revision = [...run.revisions].reverse().find((item) => item.status === 'accepted'); if (!revision) return response(failure(run), 200, 'text/html; charset=utf-8'); if (creation[1] === 'c') return response(simpleOwnerPage(run, revision), 200, 'text/html; charset=utf-8', { 'content-security-policy': "default-src 'none'; script-src 'self'; style-src 'self'; img-src 'self' data:; connect-src 'self'; frame-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'" }); run.lastCapabilityExpiresAt = Date.now() + 600000; if (typeof store.save === 'function') await store.save(run); const token = signer.issue({ creationId: run.creationId, revision: revision.revision, selected: run.selectedApis.flatMap((entry) => entry.operationIds.map((operationId) => ({ apiId: entry.apiId, operationId }))) }); const harness = `<script>window.randomware=Object.freeze({call:async(a,o,p)=>{const r=await fetch('/api/runtime/call',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({creationId:${JSON.stringify(run.creationId)},revision:${revision.revision},apiId:a,operationId:o,params:p,capability:${JSON.stringify(token)}})});if(!r.ok)throw new Error('broker_failure');return r.json()},ready:()=>parent.postMessage({channel:'randomware',type:'ready'},'*')});</script>`; return response(`${harness}${revision.html}`, 200, 'text/html; charset=utf-8', { 'content-security-policy': "default-src 'none'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; img-src data: blob: 'self'; media-src blob: 'self'; connect-src 'self'; font-src 'none'; frame-src 'none'; worker-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'self'", 'access-control-allow-origin': 'null' }); }
      const source = url.pathname.match(/^\/api\/creations\/([^/]+)\/(source|requests|dataflow)$/); if (source && request.method === 'GET') { const run = await callStore('findByCreation', source[1]); if (source[2] === 'source') return response([...run.revisions].reverse()[0]?.html || '', 200, 'text/plain; charset=utf-8'); if (source[2] === 'requests') return response(run.runtimeRequests || []); return response(await callStore('dataflow', run.id)); }
