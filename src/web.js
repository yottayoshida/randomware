...
const { showcasePage, creationPage, failurePage, requestsPage, dataflowPage, reportPage } = require('./core/presentation');
...
function createWebHandler({ store = new RunStore(), broker = new Broker({ fixtureMode: false }), signer = new CapabilitySigner('worker-development-secret'), assets } = {}) {
  ...
  return async function handle(request, env = {}, ctx) {
    ...
    try {
      ...
      const source = url.pathname.match(/^\/api\/creations\/([^/]+)\/(source|requests|dataflow)$/); if (source && request.method === 'GET') { const run = await callStore('findByCreation', source[1]); if (source[2] === 'source') { const requested = Number(url.searchParams.get('revision')); const revision = Number.isInteger(requested) && requested > 0 ? run.revisions.find((item) => item.revision === requested) : [...run.revisions].reverse()[0]; return response(revision?.html || '', 200, 'text/plain; charset=utf-8'); } const raw = url.searchParams.get('format') === 'raw'; if (source[2] === 'requests') return raw ? response(run.runtimeRequests || []) : response(requestsPage(run), 200, 'text/html; charset=utf-8', { 'content-security-policy': "default-src 'none'; style-src 'self'; base-uri 'none'; frame-ancestors 'none'" }); const flow = await callStore('dataflow', run.id); return raw ? response(flow) : response(dataflowPage(run, flow), 200, 'text/html; charset=utf-8', { 'content-security-policy': "default-src 'none'; style-src 'self'; base-uri 'none'; frame-ancestors 'none'" }); }
      const moderation = url.pathname.match(/^\/api\/creations\/([^/]+)\/(report|unpublish)$/); if (moderation) { const run = await callStore('findByCreation', moderation[1]); if (moderation[2] === 'report' && request.method === 'GET') return response(reportPage(run), 200, 'text/html; charset=utf-8', { 'content-security-policy': "default-src 'none'; style-src 'self'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'" }); if (moderation[2] === 'report' && request.method === 'POST') { const isForm = request.headers.get('content-type')?.includes('application/x-www-form-urlencoded'); const input = isForm ? await readForm(request) : await readJson(request); await callStore('reportCreation', run.creationId, input.reason || 'unspecified'); return isForm ? response(reportPage(run, true), 200, 'text/html; charset=utf-8', { 'content-security-policy': "default-src 'none'; style-src 'self'; base-uri 'none'; frame-ancestors 'none'" }) : response({ ok: true, status: 'hidden' }); } if (moderation[2] === 'unpublish' && request.method === 'POST') { const expected = env.RANDOMWARE_OWNER_TOKEN || 'local-owner-token'; if (request.headers.get('authorization') !== `Bearer ${expected}`) throw new Error('owner_auth_required'); await callStore('unpublishCreation', run.creationId); return response({ ok: true, status: 'unpublished' }); } }
