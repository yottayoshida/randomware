...
function createWebHandler({ store = new RunStore(), broker = new Broker({ fixtureMode: false }), signer = new CapabilitySigner('worker-development-secret'), assets } = {}) {
  ...
  return async function handle(request, env = {}, ctx) {
    ...
    try {
      ...
      const reroll = url.pathname.match(/^\/api\/runs\/([^/]+)\/reroll$/); if (reroll && request.method === 'POST') { await callStore('noteActivity', reroll[1]); return response(summary(await callStore('rerollConcept', reroll[1], await readJson(request)))); }
