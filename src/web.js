...
function summary(run, origin) {
  return { runId: run.id, runContract: run.runContract || `run:${run.id}`, promptVersion: 'concept-v1', conceptId: run.concept?.requestId || null, ...runUrls(run, origin), phase: run.phase, choreography: run.choreography || null, createdAt: run.createdAt, creationId: run.creationId, styleId: run.styleId || null, style: run.styleId ? getStyle(run.styleId) : null, selectedApis: run.selectedApis.map(({ apiId, operationIds }) => { const entry = registry.find((item) => item.id === apiId); return { id: apiId, name: entry.name, category: entry.category, capability: entry.capability, operations: entry.operations.filter((op) => operationIds.includes(op.id)).map((op) => ({ ...op, liveDraw: op.cacheMs === 0 })) }; }), concept: run.concept, conceptHistory: run.conceptHistory || [], failure: run.failure, revisions: run.revisions.map(({ revision, bytes, sha256, status, at }) => ({ revision, bytes, sha256, status, at })), events: run.events, repairCount: run.repairCount };
}
  ...
  return async function handle(request, env = {}, ctx) {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/mcp') {
        ...
        if (input.method === 'tools/call' && input.params?.name === 'spin_apis') { const args = input.params.arguments || {}; const spinSeed = args.seed || seed(); const selected = selectApis({ seed: spinSeed, registry, unhealthy: await unhealthy() }); const style = selectStyle({ seed: spinSeed, history: args.styleHistory || [] }); const run = await callStore('createRun', { requestId: args.requestId || seed(), selectedApis: selected.map((entry) => ({ apiId: entry.id, operationIds: entry.operations.map((op) => op.id) })), styleId: style.id, styleHistory: args.styleHistory || [] }); return response({ jsonrpc: '2.0', id: input.id, result: callToolResult(summarize(run), `Selected ${run.selectedApis.length} APIs and style ${style.name}.`) }); }
      ...
      if (url.pathname === '/api/runtime/call') {
        ...
        if (request.method === 'POST') {
          const input = await readJson(request); const run = await callStore('findByCreation', input.creationId); if (run.unpublished) throw new Error('creation_unpublished'); const capability = signer.verify(input.capability, { creationId: input.creationId, revision: input.revision, apiId: input.apiId, operationId: input.operationId }); await callStore('assertRuntimeQuota', run.id, capability.quotas); const result = await broker.call({ selectedApis: run.selectedApis, apiId: input.apiId, operationId: input.operationId, params: input.params || {}, onFetch: ({ apiId, dailyBudget }) => callStore('consumeDailyBudget', `api:${apiId}`, dailyBudget), onRetry: (retry) => callStore('logRuntime', run.id, { ...retry, bytes: 0, cacheHit: false }), media: { origin: url.origin, runId: run.id, creationId: run.creationId, revision: input.revision, capability, tokenSigner: signer, mediaStore: store } }); await callStore('logRuntime', run.id, { apiId: input.apiId, operationId: input.operationId, status: 'ok', bytes: result.bytes, cacheHit: result.cached }); return response(result, 200, 'application/json; charset=utf-8', runtimeCors);
        }
      }
      if (request.method === 'POST' && url.pathname === '/api/spin') { const input = await readJson(request); const spinSeed = input.seed || seed(); const selected = selectApis({ seed: spinSeed, registry, history: input.history || [], unhealthy: await unhealthy() }); const style = selectStyle({ seed: spinSeed, history: input.styleHistory || [] }); const run = await callStore('createRun', { requestId: input.requestId || seed(), selectedApis: selected.map((entry) => ({ apiId: entry.id, operationIds: entry.operations.map((op) => op.id) })), history: input.history || [], styleId: style.id, styleHistory: input.styleHistory || [] }); return response({ ...summarize(run), disclosure: 'Building publishes this experimental AI-generated app at a public URL.' }); }
      ...
      if (runMatch) {
        ...
        if (request.method === 'POST' && action === 'concept') { const input = await readJson(request); await callStore('noteActivity', runId, Date.now(), ['spinned']); const run = await callStore('getRun', runId); const concept = { ...input, apiIds: input.apiIds || run.selectedApis.map((entry) => entry.apiId) }; const check = validateConcept(concept, { selectedApis: run.selectedApis, prior: run.history || [], styleId: run.styleId }); if (!check.ok) return response({ ...check, ...summarize(await callStore('getRun', runId)) }, 422); return response(summarize(await callStore('acceptConcept', runId, concept))); }
